...
#This analysis is on unstranded RNA-seq data from X1 or CDS2 cells transfected with either a control siRNA or an siRNA targeting POLE.
...
#C1_NC_CDS2 - Control CDS2
#C2_siPE_CDS2 - siPOLE CDS2
#D1_NC_CDS2 - Control CDS2
#D2_siPE_CDS2 - siPOLE CDS2
...
C1 = fread("alignments/star/C1_NC_CDS2ReadsPerGene.out.tab", skip = 4)
C2 = fread("alignments/star/C2_siPE_CDS2ReadsPerGene.out.tab", skip = 4)
D1 = fread("alignments/star/D1_NC_CDS2ReadsPerGene.out.tab", skip = 4)
D2 = fread("alignments/star/D2_siPE_CDS2ReadsPerGene.out.tab", skip = 4)
...
C1_unstrand = dplyr::select(C1, c(1,2))
C2_unstrand = dplyr::select(C2, c(1,2))
D1_unstrand = dplyr::select(D1, c(1,2))
D2_unstrand = dplyr::select(D2, c(1,2))
...
colnames(C1_unstrand) = c("gene","CDS2_control_1")
colnames(C2_unstrand) = c("gene","CDS2_siPOLE_1")
colnames(D1_unstrand) = c("gene","CDS2_control_2")
colnames(D2_unstrand) = c("gene","CDS2_siPOLE_2")
...
master = inner_join(master, C1_unstrand, by = "gene")
master = inner_join(master, D1_unstrand, by = "gene")
master = inner_join(master, C2_unstrand, by = "gene")
master = inner_join(master, D2_unstrand, by = "gene")
...
#I am choosing to use a GLM for this analysis, because I want all 4 samples to be normalized together.
#So I will use the non-classic pipeline.

groups = c("X1_ctrl","X1_ctrl","X1_siPOLE", "X1_siPOLE","CDS2_control","CDS2_control","CDS2_siPOLE","CDS2_siPOLE")
dg = DGEList(counts = master, group = groups)
keep = filterByExpr(dg)
dg = dg[keep, , keep.lib.sizes = FALSE]
dg = calcNormFactors(dg)
#checking group order with dg$samples$group
#currently the order is: CDS2_control, CDS2_siPOLE, X1_ctrl, X1_siPOLE
#important for choosing coefficients later. 
design = model.matrix(~ 0 + dg$samples$group)
#can also check the design matrix here to see things are in the same order as above for making contrasts.
#design
colnames(design) = levels(dg$samples$group)
...
#compare CDS2_siPOLE to CDS2_control
qlf.CDS2_siPOLE_control = glmQLFTest(fit, contrast = c(-1,1,0,0))

#compare X1_siPOLE to X1_control
qlf.X1_siPOLE_control = glmQLFTest(fit, contrast = c(0,0,-1,1))

#now we can pull out tables of results from these tests for use in volcano plots etc.
CDS2_siPOLE_control = qlf.CDS2_siPOLE_control$table
X1_siPOLE_control = qlf.X1_siPOLE_control$table

#Let's add FDR-adjusted p-values to each of these
CDS2_siPOLE_control = dplyr::mutate(CDS2_siPOLE_control, fdr = p.adjust(PValue, method = "fdr"))
...
CDS2_siPOLE_control = tibble::rownames_to_column(CDS2_siPOLE_control, "ensembl_gene_id")
CDS2_siPOLE_control_names = left_join(CDS2_siPOLE_control, symbol_key, by = "ensembl_gene_id")
...
X1_volcano = ggplot(data = X1_siPOLE_control_names, aes(x = logFC, y = -log10(fdr))) +
  ...
  geom_point(data = X1_siPOLE_control_names[-log10(X1_siPOLE_control_names$fdr) > 3 & X1_siPOLE_control_names$logFC<0,], color = "dark blue")+
  geom_point(data = X1_siPOLE_control_names[-log10(X1_siPOLE_control_names$fdr) > 3 & X1_siPOLE_control_names$logFC>0,], color = "maroon")+
  theme_classic()+
...
CDS2_volcano = ggplot(data = CDS2_siPOLE_control_names, aes(x = logFC, y = -log10(fdr))) +
  geom_point(color = "gray")+
  geom_point(data = CDS2_siPOLE_control_names[-log10(CDS2_siPOLE_control_names$fdr) > 3 & CDS2_siPOLE_control_names$logFC<0,], color = "dark blue")+
  geom_point(data = CDS2_siPOLE_control_names[-log10(CDS2_siPOLE_control_names$fdr) > 3 & CDS2_siPOLE_control_names$logFC>0,], color = "maroon")+
  theme_classic()+
  geom_hline(yintercept = 3, linetype = "dashed", alpha = 0.5) +
  geom_text_repel(data = CDS2_siPOLE_control_names[CDS2_siPOLE_control_names$hgnc_symbol %in% c("POLE", "CDKN1A"),],
                  aes(label = hgnc_symbol), box.padding = 0.5) +
  ggtitle("CDS2 siPOLE vs control")

pdf(file = "plots/X1_volcano.pdf",width = 4, height = 4)
X1_volcano
dev.off()

pdf(file = "plots/CDS2_volcano.pdf",width = 4, height = 4)
CDS2_volcano
dev.off()

#Write the full tables to .csv files:

write.table(X1_siPOLE_control_names, file = "output_data/X1_siPOLE_control.csv", sep = ",", row.names = F)
write.table(CDS2_siPOLE_control_names, file = "output_data/CDS2_siPOLE_control.csv", sep = ",", row.names = F)
...
p53_genes_retained = p53_genes[(p53_genes %in% X1_siPOLE_control_names$hgnc_symbol) &  (p53_genes %in% CDS2_siPOLE_control_names$hgnc_symbol)]
sen_genes_retained = sen_genes[(sen_genes %in% X1_siPOLE_control_names$hgnc_symbol) &  (sen_genes %in% CDS2_siPOLE_control_names$hgnc_symbol)]
...
CDS2_volcano_pole_sen = ggplot(data = CDS2_siPOLE_control_names, aes(x = logFC, y = -log10(fdr))) +
  geom_point(color = "gray")+
  geom_point(data = CDS2_siPOLE_control_names[CDS2_siPOLE_control_names$hgnc_symbol %in% unique_p53_sen,], color = "dark blue")+
  theme_classic()+
  geom_hline(yintercept = 3, linetype = "dashed", alpha = 0.5) +
  geom_text_repel(data = CDS2_siPOLE_control_names[CDS2_siPOLE_control_names$hgnc_symbol %in% unique_p53_sen,],
                  aes(label = hgnc_symbol), box.padding = 0.5) +
  ggtitle("CDS2 siPOLE vs control")

pdf(file = "plots/CDS2_p53_sen_volcano.pdf",width = 6, height = 6)
CDS2_volcano_pole_sen
dev.off()
...
logcpm_pivot = pivot_longer(logcpm_plotting, names_to = "sample", cols = c(2:9), values_to = "logcpm")

#add a variable about what kind of sample each is
logcpm_pivot = dplyr::mutate(logcpm_pivot, Condition = ifelse(sample %in% c("X1_control_1","X1_control_2"), "X1_Control", 
                                                              ifelse(sample %in% c("X1_siPOLE_1","X1_siPOLE_2"),"X1_siPOLE",
                                                                     ifelse(sample %in% c("CDS2_siPOLE_1","CDS2_siPOLE_2"),"CDS2_siPOLE", "CDS2_Control"))))

#set the order of the condition variable
logcpm_pivot$Condition = factor(logcpm_pivot$Condition, levels = c("X1_Control","X1_siPOLE","CDS2_Control","CDS2_siPOLE"))
...
make_logcpm_plot = function(name){
  ...
  ggplot(logcpm_pivot[logcpm_pivot$Gene %in% paste(gene),], aes(x = Condition, y = logcpm, fill = Condition)) + 
    ...
    scale_fill_manual(values = c("red","pink","blue","light blue"))
}
...
CDS2_siPOLE_noNA = CDS2_siPOLE_control_names[!is.na(CDS2_siPOLE_control_names$hgnc_symbol),]
CDS2_siPOLE_noNA[CDS2_siPOLE_noNA$hgnc_symbol == "POLE",]$fdr
CDS2_siPOLE_noNA[CDS2_siPOLE_noNA$hgnc_symbol == "CDKN1A",]$fdr
CDS2_siPOLE_noNA[CDS2_siPOLE_noNA$hgnc_symbol == "SESN3",]$fdr
